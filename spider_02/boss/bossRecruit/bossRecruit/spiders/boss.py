...
class BossSpider(scrapy.Spider):
    ...
    start_urls = 'https://www.lagou.com/jobs/list_%E6%95%B0%E6%8D%AE%E5%88%86%E6%9E%90?px=default&city=%E6%88%90%E9%83%BD#filterBox'
    base_url = 'https://www.lagou.com/'
    base_city = '成都'

    def parse(self, response):

        # item = self.get_boss_list(response)
        item = self.get_lagou_list(response)
    ...
    def get_boss_detals(self ,response,item):
    ...
    def get_boss_list(self,response):
    ...
    def get_lagou_list(self,response):
        lis = response.xpath("//a[@class='position_link']")

        for li in lis[0:2]:

            href = li.xpath("./@href").extract_first()
            title = li.xpath("./text()").extract_first()
            item = dict({"链接": href, "职位名称": title})
            time.sleep(random.random() * 10)
            yield scrapy.Request(href, callback=self.parse_details, meta={"item": item}, dont_filter=True)
        nexturl = self.base_url + response.xpath("//a[@ka='page-next']/@href").extract_first()
        while nexturl:
